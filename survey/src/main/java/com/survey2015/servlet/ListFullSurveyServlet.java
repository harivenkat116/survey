 ...
 */
public class ListFullSurveyServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ListFullSurveyServlet() {
        super();
...
*/
		...
		try {
			List<FullSurveyRecord> fullSurveyRecords = dao.getAllFullSurvey();

			request.setAttribute("fullSurveyRecords", fullSurveyRecords);	// place customers in output queue.
			request.setAttribute("size", fullSurveyRecords.size());	// place customers.size() in output queue.
			...
			RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/jsp_02/list_fullsurvey.jsp");
